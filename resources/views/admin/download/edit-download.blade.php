...
<div class="wrapper row-offcanvas row-offcanvas-left">
    ...
    <aside class="right-side">
        ...
        <section class="content-header">
            <h1>
                Manage Download
                <small>add or edit Download</small>
            </h1>
            <ol class="breadcrumb">
        ...
        <section>
            <div class="col-md-12">
                ...
                <div class="box box-warning">
                    ...
                    <div class="box-body">
                        <form role="form" action="edit-download" method="post">
                            <!-- text input -->
                            <input type="hidden" name="id" value="{{$download['id'] or 0}}">
                            <div class="form-group">
                                <label>类型</label>
                                {{--1=>英皇金业,2=>英皇证券,3=>英皇期货--}}
                                <select name="type" >
                                    <option value="1"
                                            @if(!isset($download['type'])||$download['type'] == 1)  selected
                                            @endif    >英皇金业</option>
                                    <option value="2"
                                            @if(isset($download['type']) &&$download['type'] == 2)  selected
                                            @endif    >英皇证券</option>
                                    <option value="3"
                                            @if(isset($download['type']) &&$download['type'] == 3)  selected
                                            @endif    >英皇期货</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>名称</label>
                                <input type="text" class="form-control" name="table_name" placeholder="Enter ..."
                                       value="{{$download['table_name'] or ''}}" required/>
                            </div>
                            <div class="form-group">
                                <label>jpg地址</label>
                                <input type="text" class="form-control" name="jpg_url" placeholder="Enter ..."
                                       value="{{$download['jpg_url'] or ''}}"   />
                            </div>
