...
<div class="wrapper row-offcanvas row-offcanvas-left">
    ...
    <aside class="right-side">
        ...
        <section class="content-header">
            <h1>
               角色
                <small>preview of role manage</small>
            </h1>
            <ol class="breadcrumb">
                        ...
                        <div class="box-body table-responsive no-padding">
                            <table class="table table-hover">
                                ...
                                @foreach($role_list as $key=>$role)
                                    <tr>
                                        <td>{{$role['id']}}</td>
                                        <td><span class="label label-success">{{$role['name']}}</span>
                                        </td>
                                        <td>{{$role['description']}}</td>
                                        <td>{{$role['created_at']}}</td>
                                        <td><a href="{{env('APP_URL')}}/admin/edit-role?role_id={{$role['id']}}">编辑</a></td>
                                        <td><a class='del-role' role_id="{{$role['id']}}" href="#">删除</a></td>
                                    </tr>
                                @endforeach
...
<script >
    ...
    $('.del-role').click(function(){
        ...
        layer.confirm('你确定要删除角色吗？', {
            btn: ['取消删除','确定'] //按钮
        }, function(){
            layer.msg('取消删除', {icon: 1});
        }, function(){
            window.location.href="{{env('APP_URL')}}"+'/admin/del-role?role_id='+role_id;
        });
